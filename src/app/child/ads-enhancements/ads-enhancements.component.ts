...
export class AdsEnhancementsComponent implements OnInit, DoCheck {
  ...
  constructor(public fb:FormBuilder,public _upload:UploadService,public _toggle:ToggleServiceService) { }
  ...
  adsEnancementsFun(e:any,formGroupName:string,formControlName:string,lineAmount:string,percentage:string,percentageFee:string,monthlyBudget:string){
    ...
    if('percentage'==e.target.dataset.name ||'dollerP'==e.target.dataset.name){
      this.display[percentageFee]=this.display[percentage] * (this.display[monthlyBudget]||0);
      this.display[lineAmount]= this.display[percentageFee]+(this.display[monthlyBudget]||0);
    }
...
value!:any
  adsSlide(e:any){
   if('optOut'==e.source.name){
     ...
     if(e.source.checked){
      this.adsEnhancementsForm.get(['optOutOfaApolloSocial','aMothlyFee'])?.enable()
      let data=this.adsEnhancementsForm.value.optOutOfaApolloSocial.aMothlyFee
    ...
    // this.display.aLineAmount=this.value
      ...
      this.adsEnhancementsForm.get(['optOutOfaApolloSocial','aMothlyFee'])?.disable()
   ...
   if('DMS'==e.source.name){
    if(e.source.checked){
      this.adsEnhancementsForm.get(['dmsIntegration','aMothlyFee'])?.enable()
    ...
    // this.display.aLineAmount=this.value
      ...
      this.adsEnhancementsForm.get(['dmsIntegration','aMothlyFee'])?.disable()
   ...
   if('facebook'==e.source.name){
    if(e.source.checked){
      this.adsEnhancementsForm.get(['facebookMarketPlace','aMothlyFee'])?.enable()
      let data=this.adsEnhancementsForm.value.facebookMarketPlace.aMothlyFee
    // this._upload.removeDoller(data).subscribe((res:number)=>{
    // this.value=res
    // })
    ...
    // this.display.aLineAmount=this.value
      ...
      this.adsEnhancementsForm.get(['facebookMarketPlace','aMothlyFee'])?.disable()
