...
export class ServiceComponent implements OnInit,DoCheck {
  ...
  servieFun(e:any,formGroupName:string,formControlName:string,lineAmount:string,lineAmountTotal:string){
  ...
  if('sub'==e.target.dataset.name){
    this.display.lineAmountTotal1=(this.display.lineAmount||0)+(this.display.lineAmount1||0)+(this.display.lineAmount2||0)
  }
  if('Sub'==e.target.dataset.name){
    this.display.lineAmountTotal2=(this.display.lineAmount3||0)+(this.display.lineAmount4||0)+(this.display.lineAmount5||0)
  }
...
@ViewChild('#input1')input1!:ElementRef
 ...
 this.input2=this.serviceForm.value.service.monthlyFee
   ...
   servicetotal!:any 
  ngDoCheck(){
  this.servicetotal=(this.display.lineAmountTotal2||0)+(this.display.lineAmountTotal1||0)
